...
class ForgotPasswordTest(TestCase):
    def setUp(self):
        ...
        AdminProfile.objects.create(admin=self.admin_user, email='adminuser@yopmail.com')
        
        # Create Regular User
        self.regular_user = User.objects.create_user(username='regularuser', password='userpass')
        UserProfile.objects.create(user=self.regular_user, email='regularuser@yopmail.com')
        
        # Create Creator user
        self.creator_user = User.objects.create_user(username='creatoruser', password='creatorpass')
        CreatorProfile.objects.create(creator=self.creator_user, organization_email='creatoruser@yopmail.com')
    ...
    def test_password_reset_regular_user(self):
        ...
        """
        response = self.client.post(reverse('password_reset'), {'email': 'regularuser@yopmail.com'})
        # Check redirect to 'password_reset_done'
        self.assertRedirects(response, reverse('password_reset_done'))
        # Check that one email was sent
        self.assertEqual(len(mail.outbox), 1)
    ...
    def test_password_reset_creator_user(self):
        ...
        """
        response = self.client.post(reverse('password_reset'), {'email': 'creatoruser@yopmail.com'})
        # Check redirect to 'password_reset_done'
        self.assertRedirects(response, reverse('password_reset_done'))
        # Check that one email was sent
        self.assertEqual(len(mail.outbox), 1)
    ...
    def test_password_reset_non_existing_email(self):
        ...
        """
        response = self.client.post(reverse('password_reset'), {'email': 'nonexistent@yopmail.com'})
        ...
        self.assertEqual(form.errors['email'], ["No user is associated with this email address."])
    ...
    def test_password_reset_invalid_email_format(self):
        ...
        """
        response = self.client.post(reverse('password_reset'), {'email': 'invalid-email'})
        # Check that the form is re-rendered with errors
        self.assertEqual(response.status_code, 200)
        form = response.context.get('form')
        self.assertIsNotNone(form)
    ...
    def test_password_reset_no_profile_email(self):
        ...
        response = self.client.post(reverse('password_reset'), {'email': 'noprofuser@yopmail.com'})
        # Check that the form is re-rendered with errors
        self.assertEqual(response.status_code, 200)
        form = response.context.get('form')
        self.assertIsNotNone(form)
