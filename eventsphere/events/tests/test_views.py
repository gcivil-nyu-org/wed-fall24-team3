...
from events.forms import CreatorProfileForm, UserProfileForm, EventForm, TicketPurchaseForm
...
class CreatorProfileViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.creator_user = User.objects.create_user(username='creator', password='creatorpass')
...
class DeleteEventViewTest(TestCase):
    def setUp(self):
        ...
        self.other_creator_user = User.objects.create_user(username='other_creator', password='pass')
        self.other_creator_profile = CreatorProfile.objects.create(creator=self.other_creator_user)
        # Create a superuser
        self.superuser = User.objects.create_superuser(username='admin', password='adminpass')
        # Create an event
        self.event = Event.objects.create(
    ...
    def test_delete_event_unauthenticated(self):
        response = self.client.get(reverse('delete_event', args=[self.event.id]))
        login_url = reverse('login') + '?next=' + reverse('delete_event', args=[self.event.id])
        self.assertRedirects(response, login_url)
...
class BuyTicketsViewTest(TestCase):
    ...
    def test_buy_tickets_unauthenticated(self):
        self.client.logout()
        response = self.client.get(reverse('buy_tickets', args=[self.event.id]))
        login_url = reverse('login') + '?next=' + reverse('buy_tickets', args=[self.event.id])
        self.assertRedirects(response, login_url)
